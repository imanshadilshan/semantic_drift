...
Two strategies were implemented, both identifying the target region through the identical SAM-plus-CLIP procedure the Drift Score itself uses, so that measurement and mitigation agree on what counts as the target.
...
\textbf{Masked conditioning} crops the image down to just the target region, with a 32-pixel padding margin included for surrounding context, before generation. The edit is applied only to that crop, and the result is pasted back into the full frame. The model never sees the rest of the image, which makes this a constraint applied at generation time rather than afterward.
